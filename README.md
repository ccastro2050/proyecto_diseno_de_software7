...
> ## 🌐 ENTRAR AL FRONT → **http://localhost:8064**
...
> el puerto 8063 (pasa al reiniciar el PC: la API vieja revive sin su
...
> ```
> NAMES                                         STATUS                    PORTS
> proyecto_diseno_de_software7-api-facturas-1   Up 2 hours                0.0.0.0:8063->8063/tcp
> proyecto_diseno_de_software7-postgres-1       Up 2 hours (healthy)      0.0.0.0:15458->5432/tcp
> ```
>
...
```powershell
git clone https://github.com/ccastro2050/proyecto_diseno_de_software7.git
cd proyecto_diseno_de_software7
...
| Qué | Dónde |
|---|---|
| **API Facturas** — diagnóstico | http://localhost:8063/ |
| **Swagger** (documentación interactiva: ver y probar los endpoints) | http://localhost:8063/swagger |
| **FRONT** (v7) — la aplicación completa con la marca | http://localhost:8064 |
| **Entrar al front — usuario ADMINISTRADOR** | `demo@correo.com` / `Demo123!` |
| Crearlo (una sola vez, 2 comandos) | `curl.exe -X POST http://localhost:8063/api/usuario -H "Content-Type: application/json" -d "{\"email\":\"demo@correo.com\",\"contrasena\":\"Demo123!\"}"` y luego `curl.exe -X POST http://localhost:8063/api/rol-usuario -H "Content-Type: application/json" -d "{\"fkemail\":\"demo@correo.com\",\"fkidrol\":1}"` (el segundo le da el rol Administrador: sin él, el menú sale recortado) |
| Listar productos | http://localhost:8063/api/producto |
| PostgreSQL (para SQLTools/pgAdmin, opcional) | `localhost:15458` · `postgres`/`Diseno123!` |
| SQL Server (opcional — v4) | `localhost,11458` · `sa`/`Diseno123!` |
| MariaDB (opcional — v5) | `localhost:13352` · `root`/`Diseno123!` |
...
> ℹ️ Este proyecto usa los puertos 8063 y 15458: si alguno ya está ocupado
...
> IA](docs/spec_kit/versiones/v7_front_completo/GUIA_IA7.md)** — cubre los dos caminos con su prompt exacto listo
...
│
├── api_facturas/                # LA API DE LA v1 — C#/ASP.NET Core (puerto 8063)
...
│   ├── appsettings.json         # Cadena de conexión (default localhost:15458)
...
v7  el front completo: las 12 entidades + menú por roles   ← USTED ESTÁ AQUÍ
...
| [2_spec.md](docs/spec_kit/versiones/v7_front_completo/2_spec.md) | QUÉ agrega la v7 y sus criterios de aceptación |
| [3_plan.md](docs/spec_kit/versiones/v7_front_completo/3_plan.md) | CÓMO: la arquitectura del front (distinta a la del back) |
| [4_research.md](docs/spec_kit/versiones/v7_front_completo/4_research.md) | Decisiones: sesión vs JWT, Flask, la marca |
| [5_data_model.md](docs/spec_kit/versiones/v7_front_completo/5_data_model.md) | El front NO tiene datos propios (esa es la lección) |
| [6_contracts.md](docs/spec_kit/versiones/v7_front_completo/6_contracts.md) | Las PANTALLAS del front y sus rutas |
| [7_quickstart.md](docs/spec_kit/versiones/v7_front_completo/7_quickstart.md) | Arranque y smoke test del front |
| [8_tasks.md](docs/spec_kit/versiones/v7_front_completo/8_tasks.md) | Orden de construcción por fases verificables |
