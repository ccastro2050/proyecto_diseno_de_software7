...
```
front_flask/
├── Dockerfile                  ← python:3.12-slim + flask + requests (puerto 8064)
...
**Crecen:** `docker-compose.yml` (★ servicio `front-flask` :8064,
...
Servicio `front-flask`: build `./front_flask`, puerto **8064** (estudiante
8164), `API_FACTURAS_URL=http://api-facturas:8063` (el hostname interno),
`depends_on: api-facturas`, `restart: unless-stopped`. El navegador usa
`localhost:8064`; el front usa el DNS interno — nunca localhost.
