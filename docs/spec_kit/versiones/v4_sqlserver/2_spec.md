...
flowchart LR
    CLI["Cliente HTTP"]
    subgraph API["api_facturas :8063 — v4"]
    ...
    PG[("PostgreSQL :15458<br/>bdfacturas — se siembra sola")]
    SS[("SQL Server :11458<br/>la MISMA bdfacturas<br/>sembrada por sqlserver-init")]
...
- Servicio `sqlserver` (SQL Server 2022, ~2 GB de RAM) en el compose,
  puerto publicado **11458**, con healthcheck real (sqlcmd) y
