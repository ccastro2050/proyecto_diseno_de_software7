...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8063
EXPOSE 8063
...
| `EXPOSE 8063` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8064"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8063"]
    ...
    NAV -->|"localhost:8063"| APIFACTURAS
    NAV -->|"localhost:8064"| FRONTFLASK
    ...
    FRONTFLASK -->|"http://api-facturas:8063<br/>al controlador, por el NOMBRE"| APIFACTURAS
    SQLSERVERINIT -->|"espera el healthcheck,<br/>siembra y termina"| SQLSERVER
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13352"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15458"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11458"| SQLSERVER
```
...
```yaml
  postgres:
    ...
    ports:
      - "15458:5432"                 # "puerto en su PC : puerto interno"
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8063`, `localhost:15458`); entre contenedores, nombres de
...
docker network create proyecto_diseno_de_software7_default

# 2. postgres
docker run -d --name postgres --network proyecto_diseno_de_software7_default --restart unless-stopped `
  ...
  -p 15458:5432 postgres:16-alpine

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. sqlserver
docker run -d --name sqlserver --network proyecto_diseno_de_software7_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Diseno123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11458:1433 mcr.microsoft.com/mssql/server:2022-latest

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_diseno_de_software7_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Diseno123!" `
  -v "${PWD}/db:/scripts:ro" mcr.microsoft.com/mssql/server:2022-latest /scripts/init_sqlserver.sh

# 7. mariadb
docker run -d --name mariadb --network proyecto_diseno_de_software7_default --restart unless-stopped `
...
docker run -d --name api-facturas --network proyecto_diseno_de_software7_default --restart unless-stopped `
  ...
  -v /app/bin `
  -v /app/obj `
  -p 8063:8063 api-facturas

# 10. Construir la imagen de front-flask y encenderla
docker build -t front-flask ./front_flask
docker run -d --name front-flask --network proyecto_diseno_de_software7_default --restart unless-stopped `
  -e "API_FACTURAS_URL=http://api-facturas:8063" `
  -e "CLAVE_SESION=Diseno123!Sesion" `
  -v "${PWD}/front_flask:/app" `
  -p 8064:8064 front-flask
...
```yaml
URL_API: http://api-facturas:8063
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8063` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8063` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8063:8063`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8063`, el front
...
```powershell
docker network inspect proyecto_diseno_de_software7_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_diseno_de_software7` produce
la red `proyecto_diseno_de_software7_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_diseno_de_software7-front-flask-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_diseno_de_software7_default has active endpoints (name:"proyecto_diseno_de_software7-front-flask-1" …)
```
